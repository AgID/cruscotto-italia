...
export const comuneDashboard: ToolDefinition = {
  ...
  handler: async (args: Record<string, unknown>, env: Env) => {
    ...
    // Morfologia: shard separato /data/morfologia/<istat>/ (pipeline one-shot)
    // Fetch condizionale: se non disponibile non blocca la risposta
    try {
      const [mStats, mBounds] = await Promise.all([
        fetchR2Json<Record<string, unknown>>(env, `morfologia/${resolvedIstat}/${resolvedIstat}_stats.json`, { useKvCache: false }),
