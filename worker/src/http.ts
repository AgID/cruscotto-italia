...
<main class="mcp-page">
  ...
  <div class="tools">
    <div class="tool-row"><div class="tool-name">.siope</div><div class="tool-desc">Flussi di cassa comunali (MEF &mdash; Ragioneria Generale dello Stato, banca dati SIOPE gestita da Banca d&rsquo;Italia, CC BY 4.0): <strong>pagamenti</strong> e <strong>incassi</strong> per codice gestionale D.M. 9/6/2016, serie mensile, con <strong>saldo di cassa</strong>. Copertura 7.896/7.896 comuni, storico dal 2025. Sono movimenti di <em>cassa</em>, non di competenza: il saldo di cassa non &egrave; un avanzo di bilancio. Uscite in <code>voci</code> (codici <code>U&hellip;</code>), incassi in <code>entrate.voci</code> (codici <code>E&hellip;</code>).</div></div>
    ...
    <div class="tool-row"><div class="tool-name">.morfologia</div><div class="tool-desc">Morfologia del territorio da HR-DTM 5m CNR-IRPI (Zenodo DOI 10.5281/zenodo.18872933, CC BY 4.0). Campi: <code>stats</code> (elevazione min/max/media in m, pendenza media e max in gradi, % sup. &gt;15&deg; e &gt;35&deg;, esposizione prevalente e distribuzione 8 direzioni, TRI medio e max, geomorfologia r.geomorphon con % per 10 forme Pianoro/Cima/Cresta/Spalla/Dosso/Versante/Conca/Piede/Valle/Depressione, irraggiamento solare r.sun medio/min/max kWh/m&sup2;/anno) + <code>bounds</code> (west/east/south/north/cx/cy EPSG:4326). Elaborazione GRASS GIS headless. Shard separato <code>/data/morfologia/&lt;istat&gt;/&lt;istat&gt;_stats.json</code>. Disponibile per ~7.895 comuni in progressivo aggiornamento (pipeline one-shot, non aggiornato periodicamente). Campo <code>morfologia</code> assente nello shard se comune non ancora processato.</div></div>
...
export async function handleHealth(_req: Request, env: Env): Promise<Response> {
  ...
  const status = { service: "cruscotto-italia-mcp", version: "0.19.0", r2: r2Ok, kv: kvOk, timestamp: new Date().toISOString() };
